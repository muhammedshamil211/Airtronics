...
export default function TestimonialsSection() {
  ...
  return (
    <section className="py-24 bg-[#111111] relative overflow-hidden" aria-labelledby="testimonials-heading">
      <Script id="review-schema" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(reviewSchema) }} />
      {/* Background Elements */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-[#005eb8]/10 rounded-full blur-[100px] -z-0 pointer-events-none" aria-hidden="true" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-blue-900/10 rounded-full blur-[100px] -z-0 pointer-events-none" aria-hidden="true" />
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-[1200px] relative z-10">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          ...
          <div className="flex gap-2 text-white pb-2" aria-label="5 Star Rating Overview">
            <div className="flex -space-x-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-12 h-12 rounded-full border-2 border-[#111111] bg-gray-600 flex items-center justify-center overflow-hidden relative">
                  <Image src={`https://i.pravatar.cc/100?img=${i + 10}`} alt={`Client ${i}`} fill sizes="48px" className="object-cover" />
                </div>
              ))}
            </div>
            <div className="ml-4 flex flex-col justify-center">
              <div className="flex text-[#005eb8]" aria-hidden="true">
                {[...Array(5)].map((_, i) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6" role="list">
          {testimonials.map((testimonial, index) => (
            <motion.article
              ...
              className="bg-white/5 border border-white/10 rounded-3xl p-8 hover:bg-white/10 transition-colors duration-300 relative group flex flex-col"
              role="listitem"
            >
              <Quote className="absolute top-6 right-8 w-12 h-12 text-white/5 group-hover:text-[#005eb8]/20 transition-colors duration-300" aria-hidden="true" />
              
              <div className="flex gap-1 text-[#005eb8] mb-6" aria-label={`Rated ${testimonial.rating} out of 5 stars`}>
